...
2. **The dashboard** (private, business-owner-facing) — job: dense, fast, keyboard-friendly control panel. Linear/Vercel energy.
...
### Color
A neutral, near-monochrome base with a single high-intent accent. The accent on the **storefront** is the tenant's brand color (dynamic). The accent on the **dashboard** is a fixed product green (the M-Pesa/commerce association, used with discipline).

```
/* Dashboard tokens (fixed) */
...
--text-muted:    #A1A1AA
--text-subtle:   #71717A
--brand:         #16A34A   /* commerce green — the one accent */
--brand-hover:   #15803D
--danger:        #EF4444
--warning:       #F59E0B
--success:       #22C55E
...
```
/* Storefront tokens — brand is injected per-tenant at runtime */
:root {
...
**Rules:** exactly one accent in view at a time. Green (or tenant brand) is reserved for the primary CTA and success states — never for decoration. Backgrounds stay neutral.

### Typography
- **Type family:** `Inter` (or `Geist`) for UI. Tight, modern, legible at small sizes on cheap Android screens.
...
- **Weights:** 400 body, 500 UI labels, 600 headings. Avoid 700+ except for a single hero number (the price).
...
### Depth
- Almost flat. Depth comes from **1px borders and background shifts**, not heavy shadows.
- One soft shadow allowed on elevated things (modals, the checkout sheet).
- Radius 10px on cards/buttons, 14px on modals/sheets. Consistent.
...
**Buttons**
- Primary: solid `--brand`, `--brand-contrast` text, 600 weight, full-width on mobile CTAs. One per screen.
...
**Inputs**
- 1px border, focus ring in `--brand` at low opacity. No inner shadow.
...
**Cards** — flat surface, 1px border, hover raises border to a lighter neutral (dashboard) or lifts subtly (storefront product).
...
**Badges (status)** — the shared status vocabulary, same everywhere:
| State | Color | Label |
|---|---|---|
| pending / processing | `--warning` amber, subtle bg | "Pending" / "Processing" |
| paid / success | `--success` green | "Paid" |
| failed | `--danger` red | "Failed" |
| expired | muted grey | "Expired" |
...
Linear/Vercel discipline. Fast, dense, keyboard-aware.
